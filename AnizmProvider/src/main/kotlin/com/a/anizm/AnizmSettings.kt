...
class AnizmSettings(private val prefs: SharedPreferences) {
    ...
    companion object {
        ...
        fun showDialog(context: Context, settings: AnizmSettings) {
            ...
            note("On: with lazy loading, Sistenn and similar wait until the other sources have been tried, and are skipped if those already reach the target. Off: the source order above is followed exactly, even if it means waiting on the browser.")
    ...
    /** v23: put browser-only sources (Sistenn…) after the rest while lazy loading. */
